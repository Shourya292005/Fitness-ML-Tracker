...
MODEL_DATA_PATH = os.path.join(MODEL_DIR, "health_modeling_ready.csv")
...
if st.sidebar.button("Save entry"):
    ...
    st.sidebar.info("Executing pipeline...")
    log_box = st.sidebar.empty()

    try:
        cleaning_script = os.path.join(DATA_RAW, "cleaning_data.py")
        ...
        if os.path.exists(CLEANED_PATH):
            ...
            pd.read_csv(CLEANED_PATH).to_csv(MODEL_DATA_PATH, index=False)
...
if modeling_df.empty:
    ...
    with m1:
        st.metric(
            ...
            value=f"{latest_row['Body Weight']:.1f} kg",
            delta=f"{weight_delta:.1f} kg (vs last week)",
            delta_color="inverse",
        )
    with m2:
    ...
    requested_view = st.query_params.get("view", "trends")
    tab_labels = {
        "trends": "Interactive Trends",
        "ai": "AI Predictions",
        "ledger": "Historical Data Ledger",
    }
    selected_label = tab_labels.get(requested_view, tab_labels["trends"])
    ordered_labels = [selected_label] + [
        label for label in tab_labels.values() if label != selected_label
    ]
    tab_by_label = dict(zip(ordered_labels, st.tabs(ordered_labels)))
    tab_trends = tab_by_label["Interactive Trends"]
    tab_ai = tab_by_label["AI Predictions"]
    tab_ledger = tab_by_label["Historical Data Ledger"]

    with tab_trends:
        st.subheader("Custom Attribute Analytics")
        metric_options = [
            "Body Weight & 7-Day Avg",
            "Total Calories",
            "Protein (g)",
            "Carbs (g)",
            "Fat (g)",
        ...
        if selected_metric == "Body Weight & 7-Day Avg":
            fig = px.line(
                ...
                x="Date",
                y="Weight_7Day_Avg",
                title="7-Day Smoothed Body Weight Trend (kg)",
                line_shape="spline",
                color_discrete_sequence=["#2ECC71"],
            )
            fig.add_scatter(
                x=filtered_df["Date"],
                y=filtered_df["Body Weight"],
                mode="markers",
                name="Daily Weigh-In",
                marker=dict(opacity=0.5, color="#E74C3C"),
            )
            fig.update_layout(hovermode="x unified")
            st.plotly_chart(fig, use_container_width=True)

        elif selected_metric == "Total Calories":
            filtered_df["Balance_Type"] = filtered_df["Net_Calorie_Balance"].apply(
            ...
            if "Protein" in filtered_df.columns:
                fig = px.line(
                    ...
                    x="Date",
                    y="Protein",
                    title="Daily Protein Intake History (g)",
                    markers=True,
                    line_shape="spline",
            ...
            if "Carbs" in filtered_df.columns:
                fig = px.line(
                    ...
                    x="Date",
                    y="Carbs",
                    title="Daily Carbohydrate Load History (g)",
                    markers=True,
                    line_shape="spline",
            ...
            if "Fat" in filtered_df.columns:
                fig = px.line(
                    ...
                    x="Date",
                    y="Fat",
                    title="Daily Dietary Lipid Intake (g)",
                    markers=True,
                    line_shape="spline",
            ...
            if "Active Energy" in filtered_df.columns:
                fig = px.bar(
                    ...
                    x="Date",
                    y="Active Energy",
                    title="Tracked Daily Active Energy Expenditure (kcal)",
                    color_discrete_sequence=["#00D2D3"],
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Log Active Energy metrics in the optional sidebar controls to populate activity profiles.")
    ...
    with tab_ai:
        ...
        if os.path.exists(MODEL_PATH):
            try:
                ...
                if not recent.empty:
                    ...
                    with c_left:
                        ...
                        if expected_change < -0.2:
                            st.success(
                                f"Your current energy balance is reducing your mass by roughly {abs(expected_change):.2f} kg per week."
                            )
                        elif expected_change > 0.2:
                            st.warning(
                                f"Current macro energy parameters indicate a weight gain pace of +{expected_change:.2f} kg."
                            )
                        else:
                            st.info("System flags your energy matrix as running near homeostasis maintenance limits.")
                        ...
                        fig_g = go.Figure(
                            go.Indicator(
                                ...
                                gauge={
                                    ...
                                    "steps": [
                                        {
                                            "range": [latest_row["Body Weight"] - 5, latest_row["Body Weight"]],
                                            "color": "#E3F2FD",
                                        },
                                        {
                                            "range": [latest_row["Body Weight"], latest_row["Body Weight"] + 5],
                                            "color": "#FFEBEE",
                                        },
                                    ],
                                },
                ...
                st.error(f"Error compiling analytical prediction layers: {exc}")
        else:
            st.info("No trained predictive model file found. Run the training pipeline to create your model.")
